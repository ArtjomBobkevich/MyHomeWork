    ...
    public void allTimePeriod(Time time, NameOfActivity nameOfActivity) {
        ...
        for (int counter = 0; counter < time.getTimeList().size(); counter++) {
            if (!nameOfActivity.getListAllActivity().get(counter).equals("Конец")) {
                newValue = time.getTimeList().get(counter) + "-" + time.getTimeList().get(counter + 1) + " " + nameOfActivity.getListAllActivity().get(counter) + System.lineSeparator();
                timePeriod = timePeriod.append(newValue);
            } else {
                newValue = time.getTimeList().get(counter) + " " + nameOfActivity.getListAllActivity().get(counter) + System.lineSeparator() + System.lineSeparator();
                timePeriod = timePeriod.append(newValue);
            }
        }
    }
